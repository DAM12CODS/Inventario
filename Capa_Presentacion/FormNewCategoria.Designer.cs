namespace Capa_Presentacion
{
    partial class FormNewCategoria
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            btnNewCategoria = new Button();
            txtNewCategoria = new TextBox();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Rockwell", 12F, FontStyle.Regular, GraphicsUnit.Point, 0);
            label1.Location = new Point(25, 35);
            label1.Name = "label1";
            label1.Size = new Size(140, 19);
            label1.TabIndex = 0;
            label1.Text = "Nueva Categoría:";
            // 
            // btnNewCategoria
            // 
            btnNewCategoria.BackColor = Color.LightSeaGreen;
            btnNewCategoria.Font = new Font("Rockwell", 12F, FontStyle.Regular, GraphicsUnit.Point, 0);
            btnNewCategoria.ForeColor = Color.FromArgb(64, 64, 64);
            btnNewCategoria.Location = new Point(121, 93);
            btnNewCategoria.Name = "btnNewCategoria";
            btnNewCategoria.Size = new Size(90, 28);
            btnNewCategoria.TabIndex = 1;
            btnNewCategoria.Text = "Guardar";
            btnNewCategoria.UseVisualStyleBackColor = false;
            btnNewCategoria.Click += btnNewCategoria_Click;
            // 
            // txtNewCategoria
            // 
            txtNewCategoria.Font = new Font("Rockwell", 9F, FontStyle.Regular, GraphicsUnit.Point, 0);
            txtNewCategoria.Location = new Point(171, 35);
            txtNewCategoria.Name = "txtNewCategoria";
            txtNewCategoria.Size = new Size(127, 22);
            txtNewCategoria.TabIndex = 2;
            // 
            // FormNewCategoria
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(354, 146);
            Controls.Add(txtNewCategoria);
            Controls.Add(btnNewCategoria);
            Controls.Add(label1);
            FormBorderStyle = FormBorderStyle.SizableToolWindow;
            Name = "FormNewCategoria";
            StartPosition = FormStartPosition.CenterScreen;
            Text = "Nueva Categoría ";
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        private Button btnNewCategoria;
        private TextBox txtNewCategoria;
    }
}